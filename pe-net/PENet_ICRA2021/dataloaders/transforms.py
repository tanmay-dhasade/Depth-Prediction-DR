...
class Random_Roatate_Random_Crop(object):
    """
        Random rotaing and cropping the image
    """

    def __call__(self, img):
        """
        Args: img (numpy.ndarray (C x H x W)): Image to be rotated.

        Returns:
            img (numpy.ndarray (C x H x W)): Rotated image.
        """
        angle = random.uniform(0,360)
        rotated_img = skimage.transform.rotate(img, angle, resize=False, order=0)
        return RandomCrop.__call__(rotated_img)