...
class ENet(nn.Module):
    def __init__(self, args):
        ...
        """
            Loading the Transformrer model with initialized weights trained on KITTI
        """

        self.dpt = model = DPTDepthModel(
            path='/home/trdhasade/DR/DPT/weights/dpt_hybrid_kitti-cb926ef4.pt',
            scale=0.00006016,
            shift=0.00579,
            invert=True,
        ...
        self.dpt.to('cuda')
        # self.normalization_depth = NormalizeImage(mean=[0.5, 0.5, 0.5], std=[0.5, 0.5, 0.5])
        weights_init(self)
    ...
    def forward(self, input):
        ...
        # print('rgb_depth: ',rgb_depth.shape,'rgb: ',rgb.shape)
        # img = rgb.to(memory_format=torch.channels_last)
        
        img = rgb.half()
        # print("IMAGE: ",img.shape)
        depth_prediction = self.dpt.forward(img)
        # print((depth_prediction-depth_prediction.min())/(depth_prediction.max()))
        depth_prediction = (torch.nn.functional.interpolate(depth_prediction.unsqueeze(1), size=depth_prediction.shape[1:], mode="bicubic", align_corners=False).squeeze().detach().cpu().numpy())
        depth_prediction = torch.from_numpy(depth_prediction).unsqueeze(1)
        # print("depth_pred ", depth_prediction.shape)
        # print('depth_prediction: ',depth_prediction.shape)
        ...
        # print(depth_output.shape, d_depth.shape, d_conf.shape)
        # print('check_this',depth_prediction.shape)
        # print(rgb_conf.shape, d_conf.shape)
        rgb_conf, d_conf = torch.chunk(self.softmax(torch.cat((rgb_conf, d_conf), dim=1)), 2, dim=1)
        # print(rgb_conf.shape, d_conf.shape)
        # print('d_depth: ',d_depth.shape)
        # print(depth_prediction/max(depth_prediction.all()))
        # print("-------")
        # print(d_depth)
        output = rgb_conf*rgb_depth + d_conf*d_depth
        # print('output',output.shape)

        output = ((output.detach().cpu() + depth_prediction)/2).to('cuda')
        # print('shape_2: ',output.shape)
        # print('output_add',output.shape)
        # print(output)
        # exit()
